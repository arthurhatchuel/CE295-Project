function [cost] = DailyCost2(Q,L,C,month,c_sumpeak,c_sumpartpeak,c_sum, c_winpartpeak, c_win)
...
day=datenum(datetime(2014,month,1))-datenum('31-December-2013');
%dayhours=((day-1)*24+1:day*24)'
if ~isempty(find(month == summermonths,1))  %summer month
    peakhours= C==0.14726;
    partpeakhours= C==0.10714;
    cost=L_tot'*C+c_sumpeak_scaled*max(L_tot(peakhours))+...
...
elseif ~isempty(find(month == wintermonths,1))  %winter month
    ...
    cost=L_tot'*C+c_winpartpeak_scaled*max(L_tot(partpeakhours))+...
